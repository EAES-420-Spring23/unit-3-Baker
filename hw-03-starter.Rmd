...
fish_stock_yearly <- fish_stock %>%
  group_by(YEAR) %>%
  summarize(STOCK_WEIGHT_YR = sum(WEIGHT))
na.omit(fish_stock_yearly)
```
...
52 rows from 1967-2018 were kept.
...
52 rows from 1967-2018 were kept and all variables were lost except 'year' and 'stock_weight_yr_lbs.'
...
catch_data %>% 
  ...
  pivot_longer(names_to = "weight_lbs",
               values_to = "1000",
               cols = 5:6)
```
